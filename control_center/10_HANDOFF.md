...
- 2026-05-10 10:14 Docs/Report 分支补齐评分文档和技术报告草稿，记录最新 smoke test 数据：2 本教材、20 章节、106 节点、144 边、31 决策、压缩比 3.44%、1 条模拟反馈事件
...
- 2026-05-10 12:01 Build-Backend 重开验证完成：8001 监听进程确认为本项目 `uvicorn app.main:app --app-dir backend --host 127.0.0.1 --port 8001`；`BASE_URL=http://127.0.0.1:8001 ./backend/scripts/smoke_test.sh` 通过，验证 2 本教材、20 章节、106 节点、144 边、31 决策、压缩比 3.44%、1 条反馈事件、`/api/dashboard` contract 为 `dashboard.v1`。
...
- 2026-05-10 收到中期 AI 初审报告：当前分数 `56.5/100`，Top 5 为 Docker/Compose、embedding/RAG、架构文档、Prompt 工程、桑基图。Root 已决定继续按多对话分支整改，不在本对话直接吞掉所有修改；新增 `control_center/27_AI_REVIEW_FIX_DISPATCH.md` 和 `./hackctl.sh ai-review`，把整改拆为 Deploy-Docker、Docs-Prompt、Backend-RAG、Frontend-Sankey、Review-QA。
- 2026-05-10 AI 初审整改前三项已回传并经 Root 复核：Deploy-Docker 新增 `Dockerfile`、`docker-compose.yml`、`.dockerignore`、`.env.example` 和 README Docker Quick Start，Ruby YAML 结构校验通过但本机无 Docker CLI；Docs-Prompt 新增 `docs/prompts.md` 并补 Agent 架构决策对照表 / RAG Pipeline / 路线图；Backend-RAG 仅修改 `backend/app/pipeline.py`，加入关键词 + 字符 n-gram TF-IDF cosine 轻量混合检索，Root 复跑 8001 smoke test 通过。下一步开 Review-QA 终验，不再开 Frontend-Sankey，除非终验后仍有充足时间。
- 2026-05-10 14:18 Root 启用最终冲刺稳定锁：新增 `control_center/28_FINAL_SPRINT_STABILITY_LOCK.md` 和 `./hackctl.sh lock`；已完成多重备份，主备份在 `/Users/li/Documents/hackathon_backups/new_project_4_20260510-141822`，桌面副本在 `/Users/li/Desktop/黑客松备份/new_project_4_20260510-141822`。Root 后续只收四行交接、做采用/暂缓/放弃判断、更新提交清单、commit/push，不再扩展功能。
- 2026-05-10 14:2x 提前交付快照验证完成：后端 Python compileall 通过；前端 `npm run build` 通过；Root 启动 8001 并复跑 `BASE_URL=http://127.0.0.1:8001 ./backend/scripts/smoke_test.sh` 通过，最终样例为 2 本教材、20 章节、106 节点、144 边、31 决策、压缩比 3.44%、1 条反馈事件、`dashboard.v1`。安全扫描未发现已跟踪 PDF、`.env`、私钥、数据库或真实 API key；本机无 Docker CLI，仅完成 compose YAML 静态校验。
- 2026-05-10 Review-QA 最终修复验收已回传并被 Root 采纳：后端 RAG smoke test 从 WARN 关为 PASS；Docker 路径修复三点 PASS（`.env.example` 默认 `TEXTBOOK_DIR=./textbooks`、compose 挂载到 `/textbooks`、pipeline 读取环境变量）；最终安全检查 PASS；唯一剩余 Docker Desktop 实跑验证为非阻塞 WARN。Root 已释放本地 8001 监听进程。
- 2026-05-10 Deploy-Docker 分支按最新风险反馈完成低风险改进：README 新增 30 秒评审路线和 Docker/本地后端复现命令；前端首屏新增“一键评审”引导卡，把四问和公网静态 Demo / 本地 FastAPI 边界放在首屏；RAG 检索加入目录、页码、页眉噪声降权与引用清洗；语义对齐口径明确为同义词表 + 本地字符 n-gram，不包装成完整 embedding / LLM 判断。
...
15. AI 初审整改前三项已完成，下一步只开 Review-QA 终验；暂不再开 Frontend-Sankey，避免最后阶段破坏已封版前端。
16. 最终冲刺稳定锁已启用；若上下文混乱，运行 `./hackctl.sh lock`，只按该文件行动。
17. Deploy-Docker 最新补丁需要 Root 或 Review-QA 复跑：`python -m compileall backend/app`、`cd frontend && npm run build`、可选 `BASE_URL=http://127.0.0.1:8001 ./backend/scripts/smoke_test.sh`；若通过，提交并推送以刷新 GitHub Pages。
...
- `control_center/28_FINAL_SPRINT_STABILITY_LOCK.md`
