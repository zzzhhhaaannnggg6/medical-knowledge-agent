...
- GitHub CLI 已登录并已推送 Public 仓库 `zzzhhhaaannnggg6/medical-knowledge-agent`，main 当前在 `041fe02`。
- 飞书 CLI 尚未配置；如比赛需要 CLI，则运行 `lark-cli config init --new`。
- GitHub Pages 部署链接 `https://zzzhhhaaannnggg6.github.io/medical-knowledge-agent/` 已由 Actions `run 25619090600` 重建，字号/行距/导读/stagger 均已上线；本机被 `*.github.io` TLS 重置无法自验证，待用户在无痕窗口确认"拥挤感"是否已缓解。
...
- Build-Backend 原分支已死；新后端分支必须从 `./hackctl.sh backend-restart` 接入，不再依赖旧聊天。
...
9. 如果要重开 Backend，复制 `23_BACKEND_RESTART_HANDOFF.md` 或运行 `./hackctl.sh backend-restart`。
10. 如果 Root 自己要重开，复制 `24_ROOT_RESTART_HANDOFF.md` 或运行 `./hackctl.sh root-restart`。
11. 如果用户忘记怎么使用本对话，运行 `./hackctl.sh agent`；如果要看阶段表，运行 `./hackctl.sh phase`；如果要派多个 Agent，运行 `./hackctl.sh dispatch`。
