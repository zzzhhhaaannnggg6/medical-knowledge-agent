...
本文档描述当前目标架构和已验证的 MVP 行为，不虚构尚未完成的 7 本教材全量结果、公网部署链接或真实教师反馈。
...
当前后端 smoke test 的样例结果为：原始正文 65056 字符，整合后 2651 字符，压缩比 4.07%，达到 30% 目标。该结果来自 2 本教材的 MVP 页窗，不代表 7 本教材全量处理结果。
...
- 前端当前读取 `/api/dashboard`，后端当前暴露 `/api/state`、`/api/rag/query`、`/api/feedback`，需要 Build 分支统一接口。
- RAG 引用已经能返回教材、章节、页码和片段，但样例片段仍有目录噪声，需要后端继续清洗。
- GitHub Public 仓库、Vercel 部署链接、技术报告飞书链接仍待 Deploy/Report 回填。
